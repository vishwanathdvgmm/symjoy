...
def random() -> str | None:
    """
    Return a random math symbol character.
    """
...
def list() -> list[str]:
    """
    List all math symbol names (sorted).
    """
    return [s.name for s in list_by_category(_CATEGORY)]

def items() -> dict[str, str]:
    """
    Return a dict of {name: char} for math symbols.
    """
...
def exists(name: str) -> bool:
    """
    Check if a math symbol exists by name.
    """
    if not name:
        return False
...
def info(name: str) -> dict | None:
    """
    Return metadata for a math symbol.
...
def related(name: str) -> list[dict]:
    """
    Return related math symbol characters.
    """
    if not name:
        return []
