...
def random() -> str | None:
    """
    Return a random currency character.
    """
...
def list() -> list[str]:
    """
    List all currency names (sorted).
    """
    return [s.name for s in list_by_category(_CATEGORY)]

def items() -> dict[str, str]:
    """
    Return a dict of {name: char} for currencies.
    """
...
def exists(name: str) -> bool:
    """
    Check if a currency exists by name.
    """
    if not name:
        return False
...
def related(name: str) -> list[dict]:
    """
    Return related currency characters.
    """
    if not name:
        return []
